...
async function searchCombined(query, options = {}) {
  try {
    ...
    console.log(`✅ Embedding generated! Cost: $${embeddingResult.cost.toFixed(6)}, Tokens: ${embeddingResult.tokens}`);
    ...
    console.log(`\n💰 Total Embedding Cost: $${embeddingResult.cost.toFixed(6)}`);
    console.log(`📈 Model Used: ${embeddingResult.model} (Mistral AI)`);
    console.log(`🔢 Results Found: ${allResults.length} (${testCases.length} test cases, ${userStories.length} user stories)`);
    console.log(`💡 Cost Savings: ~90% cheaper than OpenAI!`);
