...
export async function rerankDocuments(query, documents, topK = 10) {
  try {
    ...
    const prompt = `You are a relevance scoring assistant. Given a query and a list of documents, score each document's relevance to the query on a scale of 0-100.
...
Instructions:
1. Evaluate each document's relevance to the query
2. Return a JSON object with a "rankings" array
3. Each ranking should have "index" (document index) and "score" (0-100 relevance score)
4. Sort by score (highest first)
5. Include only the top ${topK} most relevant documents
6. Use this exact format: {"rankings": [{"index": 0, "score": 95}, {"index": 2, "score": 87}]}
...
Response:`;
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are a relevance scoring assistant. Return only valid JSON objects with a 'rankings' array."
        },
        {
          role: "user",
          content: prompt
    ...
    const rerankedDocs = scores
      ...
      .map(item => ({
        ...
        rerankScore: item.score / 100, // Normalize to 0-1
    ...
    console.error('❌ Groq reranking error:', error.message);
